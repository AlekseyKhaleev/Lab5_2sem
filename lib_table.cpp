...
void PrintTable(PrsPtr Persons[], int rows_request, int row_index) {
    ...
    * Исходные данные:
    *      Persons      - массив умных (shared) указателей интерфейсного класса IPerson
    ...
    * Вызываемые модули: "lib_classes.h", "lib_definitions.h"
    * Описание алгоритма:
    ...
    * 3) На каждой итерации цикла осуществляется вызов функции PrintInstance для элемента массива Persons с индексом j
    * с аргументом start_row = i. Операция приводит к выводу данных сотрудника с номером [j] в i-той строке таблицы.
...
int CreateTable(char *argv[], std::fstream &table_out, int rows_request) {
    ...
    * Вызываемые модули: "lib_classes.h", "lib_definitions.h", "lib_draw.h", "lib_print.h", <fstream>
    * Описание алгоритма:
    ...
    * 8) Объявляется массив умных (shared) указателей интерфейсного класса IPerson
    ...
    *       - с помощью функции InitInstance осуществляется вызов функции-фабрики класса IPerson, создается и
    *      инициализируется экземпляр нужного класса-наследника IPerson в соответствующих строках/столбцах таблицы
    *       - с помощью функции WriteInstance инициализированный экземпляр класса записывается в двоичный файл
    *   Если достигнута последняя строка:
    *       - заново рисуется пустой шаблон таблицы
    *       - с помощью функции PrintTable() строки таблицы, кроме последней, заполняются элементами массива Persons
    *       - с помощью функции InitInstance осуществляется вызов функции-фабрики класса IPerson, создается и
    *      инициализируется экземпляр нужного класса-наследника IPerson в соответствующих строках/столбцах таблицы
    *       - с помощью функции WriteInstance инициализированный экземпляр класса записывается в двоичный файл
    *
    ...
    *******************************************************************************************************************/
    ...
    try {
        ...
        PrsPtr Persons[rows_request];    // объявление массива указателей интерфейсного класса
...
void ViewTable(char *argv[], std::fstream &table_in, int rows_request, int row_index) {
    ...
    * 6) Объявляется массив Persons умных (shared) указателей интерфейсного класса IPerson размером rows_to_print
    * 7) В цикле осуществляется чтение данных из файла и инициализация ими соответствующих элементов массива Persons
...
void GenerateTablePage(PrsPtr Persons[], char* argv[], int rows_to_print, int real_rows, int row_index){
    ...
    * Исходные данные:
    *      Persons       - массив экземпляров-наследников интерфейсного класса IPerson, содержащий данные таблицы
    *      argv          - массив аргументов командной строки, с которыми была запущена программа
