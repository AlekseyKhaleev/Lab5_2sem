...
PrsPtr IPerson::CreateInstance(int id) {
    switch (id / 1000) {
        case 11:
            return std::make_shared<Teacher>();
        case 12:
            return std::make_shared<Assistant>();
        case 13:
            return std::make_shared<HouseKeeper>();
        case 21:
            return std::make_shared<Bachelor>();
        case 22:
            return std::make_shared<Master>();
        case 23:
            return std::make_shared<Graduate>();
        default:
            throw SyntaxException("Enter correct ID or press 'h' for help", 10);
    }
}
